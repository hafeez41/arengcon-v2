...
export function useEffectiveProjects() {
  ...
  return useMemo(() => {
    // Public order mirrors the admin's array order (drag-reorder in admin =
    // same order on the site). New entries are unshifted by upsertProject,
    // so freshly created projects still appear first.
